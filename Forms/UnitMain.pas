...
{$R *.dfm}
const
  cAppTitle = '%s%s Ч Path of Exile Loot Filter Editor';
  FilterItemPadding = 4;
  FilterTextPadding = 2;
  dfHeaderFont = 16;
  dfBaseFont = 10;
...
{*
  »з-за сломанной механики ListBox, мы храним указатель на структуру в тексте элемента списка. ќбъект добавл€етс€ к элементу отдельно, а не сразу из-за чего часть сообщений от системы
   приходит раньше, и не получаетс€ получить объект ссылку на фильтр.
*}
...
begin
  ...
  begin
    ...
    // не принимаем секцию в саму секцию и еЄ блоки, и блок в секцию в которой он и находитс€
